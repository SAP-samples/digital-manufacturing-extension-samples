sap.ui.define([
	...
	return Controller.extend("com.sap.me.customOperationPOD.controller.MainView", {
		...
		*/
		//IOXP assist
		handleIOXPAssist: function () {
			var sfc = that.getView().byId("idWorkList").getSelectedItem().getBindingContext(
				"workListModel").getObject().sfc;
			var param = [{
				name: 'Transaction',
				value: 'HMI_2020/OIF/Production/IOXP/BringToFrontPush'
			}, {
				name: 'sfc',
				value: sfc
			}];
			var url = '/XMII/Runner';
			that._executeAPI(param, 'POST', url).then(function (dataout) {
					MessageToast.show("IOXP Success - SFC: " + sfc, {
						duration: 500
					});
				})
				.catch(function (error) {
		...
		//IOXPAssist on Key Press F9(120)
		handleIOXPAssistKeyPress: function () {
				if (that.getView().byId("idWorkList").getSelectedItem()) {
					that.handleIOXPAssist();
				}
			}
			/*
					_handleFormID: function () {
						var url = '/manufacturing-rest/web-api/xsrf/generateFormId';
						that._executeAPIGET('GET', url).then(function (data) {
								that.formID = data.FORM_ID;
								that._loadWorkList();
							})
							.catch(function (error) {
		...
		_loadUserDataOld: function () {
			var param = [{
				name: 'QueryTemplate',
				value: 'customOperationPOD/Query/getUserDetails_XQry'
			}, {
				name: 'content-type',
				value: 'text/json'
			}];
			...
			that._executeAPI(param, 'POST', url).then(function (dataout) {
					var row = dataout.Rowsets.Rowset[0].Row;
					if (row) {
						var data = that.headingModel.getData();
						data.User = row[0].FirstName.substring(0, 1) + row[0].LastName.substring(0, 1);
		...
		_loadComponentListNew: function () {
			var param = {
				"sfcList": [{
					"sfcRef": "SFCBO:" + this.parametersModel.getData().Site + "," + that.getView().byId("idWorkList").getSelectedItem().getBindingContext(
						"workListModel").getObject().sfc
				}],
				"operationRef": "OperationBO:" + this.parametersModel.getData().Site + "," + this.parametersModel.getData().Operation + "," +
					this.parametersModel
					.getData().OperationVersion,
				"enforceAssyStates": true,
				"groupQuantityMode": "SUM",
				"operationRequired": true,
				"excludeCoProduct": true,
				"excludeByProduct": true,
				"includeRemoved": false,
				"includeReplaced": false,
				"includeNonBomComponents": false,
				"processLotRef": "string"
			};
			var url = "/manufacturing-rest/web-api/RetrieveComponentsServiceRPC/findAssembledAndUnfilledComponentsByProcessLot?Site=" + this.parametersModel
				.getData().Site + "FORM_ID=" + that.formID;

			that._executeAPIPOST(JSON.stringify(param), 'POST', url).then(function (data) {
					var componentsData = new Array();
					$.each(data.bomComponents, function (index, bomComponent) {
						var component = bomComponent.componentRef.slice(bomComponent.componentRef.indexOf(",") + 1, bomComponent.componentRef.lastIndexOf(
							","));
						var componentsObject = {
							"bomCompRef": bomComponent.bomComponentRef,
							"component": component
						};
						componentsData.push(componentsObject);
					});
				})
				.catch(function (error) {
					that.componentModel.setData([]);
		...
		_loadDataCollection: function () {
					...
					param.operationRef = "OperationBO:" + this.parametersModel.getData().Site + "," + this.parametersModel.getData().Operation + "," +
						this.parametersModel.getData().OperationVersion;
					param.resourceRef = "ResourceBO:" + this.parametersModel.getData().Site + "," + this.parametersModel.getData().Resource;
					sfcRef.sfcRef = "SFCBO:" + this.parametersModel.getData().Site + "," + that.getView().byId("idWorkList").getSelectedItem().getBindingContext(
						"workListModel").getObject().sfc;
					param.workCenterRef = "WorkCenterBO:" + this.parametersModel.getData().Site + "," + that.getView().byId("idWorkList").getSelectedItem()
						.getBindingContext(
							"workListModel").getObject().workCenter;
					var sfcList = new Array(sfcRef);
					param.sfcList = sfcList;

					//var url = "/manufacturing-rest/web-api/DataCollectionServiceRPC/findDcGroupsToCollect2?Site=" + this.parametersModel.getData().Site +
					//	"&FORM_ID=" + that.formID;

					var url = "/manufacturing-rest/web-api/DataCollectionServiceRPC/findDcGroupsToCollect?Site=" + this.parametersModel.getData().Site +
						"&FORM_ID=" + that.formID;
			...
			var param = [{
				name: 'QueryTemplate',
				value: 'customOperationPOD/Query/getParametricData_SQLQry'
			}, {
				name: 'content-type',
				value: 'text/json'
			}, {
				name: 'Param.1',
				value: event.getSource().getParent().getBindingContext("dataCollectionModel").getObject().dcGroupRef
			}, {
				name: 'Param.2',
				value: "OperationBO:" + this.parametersModel.getData().Site + "," + this.parametersModel.getData().Operation + "," + this.parametersModel
					.getData().OperationVersion
			}, {
				name: 'Param.3',
				value: "ResourceBO:" + this.parametersModel.getData().Site + "," + this.parametersModel.getData().Resource
			}, {
				name: 'Param.4',
				value: "SFCBO:" + this.parametersModel.getData().Site + "," + that.getView().byId("idWorkList").getSelectedItem().getBindingContext(
					"workListModel").getObject().sfc
			}];
			var url = '/XMII/Illuminator';
			that._executeAPI(param, 'POST', url).then(function (dataout) {
					...
					$.each(dcParamList, function (index, dcParam) {
						dcParamList[index].valueNotCollected = true;
						if (row) {
							$.each(row, function (i, parametricMeasure) {
								if (dcParam.parameterName === parametricMeasure.MEASURE_NAME) {
									dcParamList[index].valueNotCollected = false;
									dcParamList[index].enteredValue = parametricMeasure.ORIGINAL_ACTUAL;
									dcParamList[index].enteredComment = parametricMeasure.ORIGINAL_DC_COMMENT;
								}
							});
						}
						if (!(dcParamList[index].valueNotCollected)) {
