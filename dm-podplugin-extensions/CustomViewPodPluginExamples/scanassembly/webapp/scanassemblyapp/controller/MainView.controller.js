sap.ui.define([
	...
	return PluginViewController.extend("com.custom.plugins.scanassembly.scanassemblyapp.controller.MainView", {
		onInit: function() {
		...
		assembleMaterial: function(payload, bomComponent, scanType) {
			...
			var that = this;
			this.getPodController()._oPodController.ajaxPostRequest(url, assemblyRequestJSON,
		...
		getSFCDetails: function(payload) {
			let sfcDetail = jQuery.ajax({
				...
				url: this.getPodController()._oPodController.getPublicApiRestDataSourceUri() + "/sfc/v1/sfcdetail?plant=" + payload.plant + "&sfc=" + payload.sfc,
				async: false
			});
			return sfcDetail;
		},
